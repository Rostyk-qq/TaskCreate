...
const RegistrationForm = () => {
    ...
    const paramsUser = TypedUseSelector(state => state.UserReducer);
    ...
    const StoreParams = (e: any) => {
        ...
        CreateConnection(paramsUser)
    }

    return (
        <form>
            <input value={params.name} onChange={e => setParams({...params, name: e.target.value})} className="input"  placeholder='Name' />
            <input value={params.lastName} onChange={e => setParams({...params, lastName: e.target.value})} className="input" placeholder='Last' />
